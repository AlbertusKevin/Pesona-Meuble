...
@section('content')
    <div class="container">
        @include('message')
        <h1 class="text-center pt-5 pb-5 heading" id="{{ $numSO }}">Claim Warranty: {{ $numSO }}</h1>
                    ...
                    <div class="card-body pt-4">
                        @if (count($data) == 0)
                            <div class="modal-header">
                                <h2>No items available: items have been registered</h2>
                            </div>
                        @else
                            <div class="modal-header">
                                <h2>Line Items Available</h2>
                            </div>
                            <form action="/warranty/{{ $numSO }}" method="POST">
                                @csrf
                                @foreach ($data as $item)
                                    <div class="card" style="width: 100%;" id="{{ $item->modelType }}">
                                        <div class="row pt-3">
